...
const db = require('./db'); // SQLite — banco local, existe apenas enquanto a app roda
...
function loadCharacter(id) {
  ...
  const features = db.prepare(`SELECT id, name, description FROM character_features WHERE characterId = ?`).all(id);
  const spells = db.prepare(`SELECT id, name, school, level, casted FROM character_spells WHERE characterId = ?`).all(id);
...
(function migrateJsonIfPresent() {
  ...
  try {
    ...
    const insertChar = db.prepare(`
      ...
      VALUES (@id, @name, @class, @race, @level, @attributes, @skillProficiencies,
              @saveProficiencies, @resources, @items, @spellSlotsUsage, @imageUrl, @createdAt, @updatedAt)
    `);
    const insertFeature = db.prepare(`INSERT INTO character_features (id, characterId, name, description) VALUES (@id, @characterId, @name, @description)`);
    const insertSpell = db.prepare(`INSERT INTO character_spells (id, characterId, name, school, level, casted) VALUES (@id, @characterId, @name, @school, @level, @casted)`);

    db.transaction(() => {
      for (const c of characters) {
        insertChar.run({
          id: c.id, name: c.name, class: c.class || '', race: c.race || '', level: c.level ?? 1,
          attributes: JSON.stringify(c.attributes || {}),
          skillProficiencies: JSON.stringify(c.skillProficiencies || {}),
          saveProficiencies: JSON.stringify(c.saveProficiencies || {}),
          resources: JSON.stringify(c.resources || []),
          items: JSON.stringify(c.items || []),
          spellSlotsUsage: JSON.stringify(c.spellSlotsUsage || {}),
          imageUrl: c.imageUrl || null,
          createdAt: c.createdAt || new Date().toISOString(),
          updatedAt: c.updatedAt || new Date().toISOString(),
        });
        for (const f of (c.features || [])) {
          insertFeature.run({ id: f.id || newId(), characterId: c.id, name: f.name, description: f.description || '' });
        }
        for (const s of (c.spells || [])) {
          insertSpell.run({ id: s.id || newId(), characterId: c.id, name: s.name, school: s.school || '', level: s.level ?? 1, casted: s.casted ? 1 : 0 });
        }
      }
    })();
...
app.get('/api/paladin/spell-slots', (req, res) => {
  ...
  const row = db.prepare(`SELECT slots FROM spell_slots WHERE classId = (SELECT id FROM classes WHERE slug='paladino') AND level = ?`).get(level);
  res.json({ level, slots: row ? JSON.parse(row.slots) : { '1': 0 } });
});

app.get('/api/paladin/proficiency', (req, res) => {
  const level = Math.min(20, Math.max(1, Number(req.query.level) || 1));
  const row = db.prepare('SELECT bonus FROM proficiency_bonus WHERE level = ?').get(level);
...
app.get('/api/classes', (req, res) => {
  res.json(db.prepare('SELECT id, slug, name, icon, hitDie, primaryAbility, casting, castingAbility, role, description FROM classes ORDER BY name').all());
});

// Uma classe com todas as features
app.get('/api/classes/:slug', (req, res) => {
  const cls = db.prepare('SELECT * FROM classes WHERE slug = ?').get(req.params.slug);
  if (!cls) return res.status(404).json({ error: 'Classe não encontrada' });
  const features = db.prepare(`SELECT name, level, description FROM class_features WHERE classId = ? ORDER BY level, name`).all(cls.id);
  const slots = db.prepare(`SELECT level, slots FROM spell_slots WHERE classId = ? ORDER BY level`).all(cls.id);
...
app.get('/api/spell-slots', (req, res) => {
  const cls = db.prepare(`SELECT id, slug, name, casting FROM classes WHERE slug = ?`).get(req.query.class || 'paladino');
  if (!cls) return res.status(404).json({ error: 'Classe não encontrada' });
  const level = Math.min(20, Math.max(1, Number(req.query.level) || 1));
  const row = db.prepare('SELECT slots FROM spell_slots WHERE classId = ? AND level = ?').get(cls.id, level);
...
app.get('/api/characters', (req, res) => {
  const rows = db.prepare(`SELECT id, name, class, race, level, imageUrl FROM characters ORDER BY updatedAt DESC`).all();
  res.json(rows);
});
...
app.post('/api/characters', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO characters (id, name, class, race, level, attributes, skillProficiencies,
                            saveProficiencies, resources, items, spellSlotsUsage, imageUrl, createdAt, updatedAt)
    VALUES (@id, @name, @class, @race, @level, @attributes, @skillProficiencies,
            @saveProficiencies, @resources, @items, @spellSlotsUsage, @imageUrl, datetime('now'), datetime('now'))
  `).run({
    id: character.id, name: character.name, class: character.class, race: character.race, level: character.level,
    attributes: JSON.stringify(character.attributes),
    skillProficiencies: JSON.stringify(character.skillProficiencies),
    saveProficiencies: JSON.stringify(character.saveProficiencies),
    resources: JSON.stringify(character.resources),
    items: JSON.stringify(character.items),
    spellSlotsUsage: JSON.stringify(character.spellSlotsUsage),
...
app.put('/api/characters/:id', (req, res) => {
  ...
  db.prepare(`
    UPDATE characters SET name = @name, class = @class, race = @race, level = @level,
                          attributes = @attributes, skillProficiencies = @skillProficiencies,
                          saveProficiencies = @saveProficiencies, resources = @resources,
                          items = @items, spellSlotsUsage = @spellSlotsUsage,
                          imageUrl = @imageUrl, updatedAt = datetime('now')
    WHERE id = @id
  `).run({
    id: updated.id, name: updated.name, class: updated.class, race: updated.race, level: updated.level,
    attributes: JSON.stringify(updated.attributes),
    skillProficiencies: JSON.stringify(updated.skillProficiencies),
    saveProficiencies: JSON.stringify(updated.saveProficiencies),
    resources: JSON.stringify(updated.resources),
    items: JSON.stringify(updated.items),
    spellSlotsUsage: JSON.stringify(updated.spellSlotsUsage),
...
app.delete('/api/characters/:id', (req, res) => {
  const existing = db.prepare('SELECT imageUrl FROM characters WHERE id = ?').get(req.params.id);
  if (!existing) return res.status(404).json({ error: 'Personagem não encontrado' });
...
app.post('/api/characters/:id/image', upload.single('image'), (req, res) => {
  const character = db.prepare('SELECT id, imageUrl FROM characters WHERE id = ?').get(req.params.id);
  if (!character) return res.status(404).json({ error: 'Personagem não encontrado' });
  ...
  db.prepare("UPDATE characters SET imageUrl = ?, updatedAt = datetime('now') WHERE id = ?").run(`/uploads/${req.file.filename}`, character.id);
...
app.delete('/api/characters/:id/image', (req, res) => {
  ...
  db.prepare("UPDATE characters SET imageUrl = NULL, updatedAt = datetime('now') WHERE id = ?").run(character.id);
  res.json({ ok: true });
});
...
app.post('/api/characters/:id/features', (req, res) => {
  ...
  db.prepare('INSERT INTO character_features (id, characterId, name, description) VALUES (?, ?, ?, ?)')
    .run(feature.id, character.id, feature.name, feature.description);
  res.status(201).json(feature);
});

app.put('/api/characters/:id/features/:featureId', (req, res) => {
  const feature = db.prepare('SELECT * FROM character_features WHERE id = ? AND characterId = ?')
    .get(req.params.featureId, req.params.id);
  ...
  db.prepare('UPDATE character_features SET name = ?, description = ? WHERE id = ?').run(name, description, feature.id);
  res.json({ id: feature.id, name, description });
});

app.delete('/api/characters/:id/features/:featureId', (req, res) => {
  const feature = db.prepare('SELECT id FROM character_features WHERE id = ? AND characterId = ?')
    .get(req.params.featureId, req.params.id);
  if (!feature) return res.status(404).json({ error: 'Feature não encontrada' });
...
app.post('/api/characters/:id/resources', (req, res) => {
  ...
  db.prepare("UPDATE characters SET resources = ?, updatedAt = datetime('now') WHERE id = ?")
    .run(JSON.stringify(character.resources), character.id);
...
app.post('/api/characters/:id/items', (req, res) => {
  ...
  db.prepare("UPDATE characters SET items = ?, updatedAt = datetime('now') WHERE id = ?")
    .run(JSON.stringify(character.items), character.id);
...
app.post('/api/characters/:id/spells', (req, res) => {
  ...
  db.prepare('INSERT INTO character_spells (id, characterId, name, school, level, casted) VALUES (?, ?, ?, ?, ?, ?)')
    .run(spell.id, character.id, spell.name, spell.school, spell.level, spell.casted ? 1 : 0);
  res.status(201).json(spell);
});

app.put('/api/characters/:id/spells/:spellId', (req, res) => {
  const spell = db.prepare('SELECT * FROM character_spells WHERE id = ? AND characterId = ?')
    .get(req.params.spellId, req.params.id);
  ...
  db.prepare('UPDATE character_spells SET name = ?, school = ?, level = ?, casted = ? WHERE id = ?')
    .run(name, school, level, casted ? 1 : 0, spell.id);
  res.json({ id: spell.id, name, school, level, casted });
});

app.delete('/api/characters/:id/spells/:spellId', (req, res) => {
  const spell = db.prepare('SELECT id FROM character_spells WHERE id = ? AND characterId = ?')
    .get(req.params.spellId, req.params.id);
  if (!spell) return res.status(404).json({ error: 'Feitiço não encontrado' });
  db.prepare('DELETE FROM character_spells WHERE id = ?').run(spell.id);
  res.json({ ok: true });
});
