...
// fica inertemente no disco, sem nenhum processo separado rodando.
...
// Configurações de segurança/transação
...
db.exec(`
  CREATE TABLE IF NOT EXISTS classes (
    ...
    hitDie TEXT,
    primaryAbility TEXT,
    casting TEXT,                 -- 'none' | 'full' | 'half' | 'pact'
    castingAbility TEXT,
    role TEXT,
    description TEXT,
  ...
  CREATE TABLE IF NOT EXISTS spell_slots (
    classId INTEGER NOT NULL REFERENCES classes(id) ON DELETE CASCADE,
    level INTEGER NOT NULL,
    slots TEXT NOT NULL,          -- JSON: {"1": 2, "2": 1}
...
{
  const stmt = db.prepare('INSERT OR IGNORE INTO proficiency_bonus (level, bonus) VALUES (@level, @bonus)');
  const insertMany = db.transaction(() => {
    for (let l = 1; l <= 20; l++) stmt.run({ level: l, bonus: PROFICIENCY[l] });
  });
  insertMany();
}
...
const FEATURES_SEED = {
  ...
  guerreiro: [
    ...
    { name:'Arquétipo Marcial', level:3, description:'Escolhe sua especialização: Campeão (acertos críticos em 19-20), Mestre de Batalha (manobras de combate com dados de superioridade) ou Cavaleiro Arcano (combina magia com combate).' }
  ],
  ladino: [
...
// full caster: 1,2,3,4,4,4,4,4,4... | half caster: começa nv 2 | pact: 1,2,2,2,3,3,4...
function buildSpellSlots() {
  const out = {};
  // full casters: bard, cleric, druid, sorcerer, wizard (slots até 9º círculo)
  const FULL = {
  ...
  // half casters: paladin, ranger (começam no nível 2, slots até 5º círculo)
  const HALF = {
  ...
  // pact caster: warlock (todos os slots do mesmo nível máximo, recarga em descanso curto)
  const PACT = {
...
const insertClasses = db.transaction(() => {
  const classStmt = db.prepare(`
    INSERT OR IGNORE INTO classes (slug, name, icon, hitDie, primaryAbility, casting, role, description, castingAbility)
    VALUES (@slug, @name, @icon, @hitDie, @primaryAbility, @casting, @role, @description, @castingAbility)
  `);
  const featureStmt = db.prepare(`
    INSERT OR IGNORE INTO class_features (classId, name, level, description)
    VALUES (@classId, @name, @level, @description)
  `);
  const slotStmt = db.prepare(`
    INSERT OR IGNORE INTO spell_slots (classId, level, slots)
    VALUES (@classId, @level, @slots)
  `);
  const getSlug = db.prepare('SELECT id FROM classes WHERE slug = ?');
  const slotTables = buildSpellSlots();

  for (const c of CLASSES_SEED) {
    classStmt.run({ ...c, castingAbility: c.castingAbility || null });
    const row = getSlug.get(c.slug);
    if (!row) continue;
    // features
    for (const f of (FEATURES_SEED[c.slug] || [])) {
      featureStmt.run({ classId: row.id, name: f.name, level: f.level, description: f.description });
    }
    // spell slots
    if (c.casting !== 'none') {
      const table = slotTables[c.casting] || slotTables.half;
      for (let l = 1; l <= 20; l++) {
        slotStmt.run({ classId: row.id, level: l, slots: JSON.stringify(table[l]) });
      }
    }
  }
});

// Roda o seed apenas se as classes ainda não existirem
(function seedIfEmpty() {
  const count = db.prepare('SELECT COUNT(*) AS n FROM classes').get().n;
  if (count === 0) insertClasses();
})();
